  ...
  for (i in 1:length(proportions) ){
    ...
    for (j in 1:num_test){
      ...
      if (variable == "random"){   
        ...
        ee <- tryCatch(model <- randomForest (formula("dep_class_7 ~ spatial_lag_squares + calls + region_based_rate + page_rank + eigen_cent + entropy"), data=trainset, mtry=3,importance=TRUE, proximity=TRUE, na.action=na.omit),
                       error = function(e){
                         print("error in RF...")
                       })
        if(inherits(ee, "error")) next
