...
draw_comb2 <- function (column){
  dd <- cbind(density$train, density[,column], spatial_lag[,column], cdr_k2[,column], 
              cdr_k3[,column], cdr_lag[,column], cdr_lag_sq[,column])
  colnames(dd) <- c("train", "density", "lag", "CDR + 2-cluster", 
                    "CDR + 3-cluster","cdr+lag", "cdr+lag_sq")
  dd <- as.data.frame(dd)
  df <- melt(dd, id.vars='train')
  colindex <- round(as.integer(as.factor(df$variable) ))
...
# OSM Graph Drawing
draw_graph <- function (column){
  dd <- cbind(density$train, spatial_lag[,column], density[,column], betweenness[,column], closeness[,column], street_network[,column], street_oa[,column])
  colnames(dd) <- c("train","spatial_lag", "density", "betweenness", "closeness", "street_network", "street+offering")
...
#Final
draw_comb3 <- function (column){
  dd <- cbind(density$train, density[,column], spatial_lag[,column],street_oa_lag[,column], 
              street_lag[,column], final[,column], final2[,column] )
