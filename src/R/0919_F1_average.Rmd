...
#       trainset$dep_class_7 <- factor(trainset$dep_class_7)  
      ...
      } else if (variable == "cdr_lag_sq"){
        ee <- tryCatch(model <- randomForest (formula("dep_class_7 ~ calls + region_based_rate + page_rank + eigen_cent + entropy + spatial_lag_square"), data=trainset,na.action=na.omit, mtry = 3, importance=TRUE, proximity=TRUE),
                       error = function(e) {
                                  print("error in RF..")
                                })
        if(inherits(ee, "error")) next
        testset <- testset[!is.na(testset$spatial_lag_square) & !is.na(testset$page_rank) ,]
...
spatial_lag_sq <- calculate_rf_table ("spatial_lag_sq")
...
cdr_lag_sq <- calculate_rf_table ("cdr_lag_sq")
...
draw_graph_base <- function (column){
  dd <- cbind(density$train, density[,column], spatial_lag[,column], spatial_lag_sq[,column] )
  colnames(dd) <- c("train", "density", "spatial_lag", "spatial_lag_sq")
...
write.csv(spatial_lag_sq, "data/results/average_predictions/spatial_lag_sq.csv")
...
# Drawings
```{r}
past <- read.csv("data/results/average_predictions/past.csv")
density <- read.csv("data/results/average_predictions/density.csv")
spatial_lag <- read.csv("data/results/average_predictions/spatial_lag.csv")
spatial_lag_sq <- read.csv("data/results/average_predictions/spatial_lag_sq.csv")
intro <- read.csv("data/results/average_predictions/intro.csv")
eigen_cent <- read.csv("data/results/average_predictions/eigen_cent.csv")
entropy <- read.csv("data/results/average_predictions/entropy.csv")
page_rank <- read.csv("data/results/average_predictions/page_rank.csv")
cdr <- read.csv("data/results/average_predictions/cdr.csv")
vol <- read.csv("data/results/average_predictions/vol.csv")
past_cdr <- read.csv("data/results/average_predictions/past_cdr.csv")
past_lag <- read.csv("data/results/average_predictions/past_lag.csv")
past_lag_cdr <- read.csv("data/results/average_predictions/past_lag_cdr.csv")
cdr_lag <- read.csv("data/results/average_predictions/cdr_lag.csv")
cdr_k2 <- read.csv("data/results/average_predictions/cdr_k2.csv")
cdr_k3 <- read.csv("data/results/average_predictions/cdr_k3.csv")
betweenness <- read.csv("data/results/average_predictions/betweenness.csv")
closeness <- read.csv("data/results/average_predictions/closeness.csv")
street_network <- read.csv("data/results/average_predictions/street_network.csv")
street_oa <- read.csv("data/results/average_predictions/street_oa.csv")
street_oa_lag <- read.csv("data/results/average_predictions/street_oa_lag.csv")
street_lag <- read.csv("data/results/average_predictions/street_lag.csv")
final <- read.csv("data/results/average_predictions/cdr_osm_lag.csv")
final2 <- read.csv("data/results/average_predictions/cdr_street_lag.csv")


draw_comb2 <- function (column){
  dd <- cbind(density$train, density[,column], spatial_lag_sq[,column],  
              cdr_k3[,column], cdr_lag[,column])
  colnames(dd) <- c("train", "density", "lag",  
                    "CDR + 3-cluster","cdr+lag")
  dd <- as.data.frame(dd)
  df <- melt(dd, id.vars='train')
  colindex <- round(as.integer(as.factor(df$variable) ))
  
  ggplot(df, aes(x = train, y = value, shape=factor(variable)
#                  colour=factor(variable))) +
    )) +
    geom_point(size = 3) +
    geom_line() +
    scale_x_continuous('\nTrain Proportion (%)',limits=c(50,90)) + 
    scale_y_continuous("",limits=c(0, 0.45)) +
    theme_bw() + 
    geom_hline(yintercept=0) + theme(legend.text=element_text(size=15)) + theme(axis.text=element_text(size=16), axis.title=element_text(size=16))
}
...
# OSM Graph Drawing
draw_graph <- function (column){
  dd <- cbind(density$train, density[,column], spatial_lag_sq[,column], betweenness[,column], closeness[,column], street_network[,column], street_oa[,column])
  colnames(dd) <- c("train","density", "spatial_lag", "betweenness", "closeness", "street_network", "street+offering")
...
#Final
draw_comb3 <- function (column){
  dd <- cbind(density$train, density[,column], spatial_lag_sq[,column],street_oa_lag[,column], 
              street_lag[,column], final[,column], final2[,column] )
  colnames(dd) <- c("train", "density", "spatial_lag","street+oa+lag", 
                    "street+lag","cdr+osm+lag", "cdr+street+lag")
  dd <- as.data.frame(dd)
  df <- melt(dd, id.vars='train')
  colindex <- round(as.integer(as.factor(df$variable) ))
